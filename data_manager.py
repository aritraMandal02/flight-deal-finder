...
class Manager(Sheety):
    ...
    def __init__(self):
        ...
        self.date_to = (dt.datetime.now() + relativedelta(months=6)).strftime('%d/%m/%Y')
        self.fly_from = flight_search.get_city_code(input('Enter departure city: '))

    def flight_search_params(self, fly_to):
        return dict(fly_from=self.fly_from, fly_to=fly_to, date_from=self.date_from, date_to=self.date_to, curr='INR', limit=1)
